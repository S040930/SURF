"""Generate descriptive visualizations for the completed r21 pilot project.
...
pilot's locked report remains technical-only (analysis_status =
pilot_technical_only); the NAE plots produced here are an explicit post-run
exploratory view requested for internal analysis.
"""
...
def database_url() -> str:
    ...
    return "postgresql+psycopg2://mac@localhost:5511/ai_marking_experiment"
...
def find_project(connection, project_id: str | None) -> dict:
    if project_id:
        row = connection.execute(
            text("select id, name, kind, status, completed_at from r21_projects where id=:id"),
            {"id": project_id},
        ).mappings().one()
    else:
        row = connection.execute(
...
def load_aggregates(engine, project_id: str | None) -> tuple[dict, list[dict], dict]:
    with engine.connect() as connection:
        ...
        raw = connection.execute(
            text("""
                select model_score, teacher_score, max_score,
                       question_id, condition, history_count,
                ...
                order by question_id, condition, history_count,
        ...
        for key, rows in cells.items():
            question, condition, history, trajectory, answer_id, group_id = key
        ...
        report_row = connection.execute(
            text("select report_json from r21_reports where project_id=:project_id order by created_at desc limit 1"),
            {"project_id": project["id"]},
        ).mappings().one()
        report = report_row["report_json"]
        resources = report["supplement"]["resources"]
        latencies = [float(v) for v in resources["latencies_ms"]]
        failures = report["supplement"].get("failures", [])

        # Memory-update constraint diagnostics: parse each succeeded snapshot.
        updates = connection.execute(
            text("""
                select condition, question_id, history_count, output_json
                from r21_calls
                where project_id=:project_id
                  and kind='memory_update'
                  and status='succeeded'
            """),
            {"project_id": project["id"]},
        ).mappings().all()
        item_counts: list[int] = []
        max_visible_tokens: list[int] = []
        for u in updates:
            out = u["output_json"]
            if not isinstance(out, dict):
                continue
            items = out.get("rules") or out.get("rubric") or []
            item_counts.append(len(items))
            tokens = _visible_tokens(out)
            max_visible_tokens.append(tokens)
        constraint = {
            "snapshot_count": len(updates),
            "max_items": max(item_counts, default=0),
            "max_visible_tokens": max(max_visible_tokens, default=0),
        ...
        operational = {
            ...
            "cli_version": resources["cli_version"],
...
def _visible_tokens(out: dict) -> int:
    """Approximate scoring-visible token count from a snapshot dict."""
    items = out.get("rules") or out.get("rubric") or []
    if not items:
        return 0
    total = 0
    for it in items:
        total += len(str(it.get("condition") or it.get("criterion") or ""))
        total += len(str(it.get("effect") or "")) + len(str(it.get("guidance") or ""))
        anchors = it.get("anchors") or {}
        total += sum(len(str(v)) for v in anchors.values())
    return total
...
def make_operational_chart(operational: dict, out: Path) -> None:
    ...
    axes[1].bar(["p50", "p95"], [operational["latency_p50_ms"] / 1000, operational["latency_p95_ms"] / 1000], color="#8C97A8")
    axes[1].set_title("Request latency", weight="bold")
    axes[1].set_ylabel("Seconds")
    axes[1].grid(axis="y", alpha=0.25)

    axes[2].bar(["Snapshots", "Max items", "Max vis. tokens"], [operational["snapshot_count"], operational["max_items"], operational["max_visible_tokens"]], color=["#2F80ED", "#F2994A", "#27AE60"])
    axes[2].set_title("Memory constraint", weight="bold")
    axes[2].grid(axis="y", alpha=0.25)

    for ax in axes:
        ax.spines[["top", "right"]].set_visible(False)
        ax.set_axisbelow(True)
    fig.suptitle("r21 pilot operational results · project 1", fontsize=15, weight="bold")
    fig.text(0.5, 0.01, f"Model={operational['model']} · {operational['cli_version']} · effort={operational['reasoning_effort']} · failures={operational['failed_calls']}",
             ha="center", fontsize=9)
    fig.tight_layout(rect=(0, 0.06, 1, 0.92))
    save_figure(fig, out / "01_operational_dashboard.png")


def select(summary: list[dict], question: str, history: int, condition: str) -> dict:
    return next(r for r in summary if r["question"] == question and r["history"] == history and r["condition"] == condition)
...
def make_final_chart(summary: list[dict], out: Path) -> None:
    ...
    for ax, question in zip(axes, QUESTIONS):
        values = [select(summary, question, 20, condition)["mean_nae"] for condition in CONDITIONS]
        bars = ax.bar(x, values, 0.55, color=[CONDITION_COLORS[c] for c in CONDITIONS])
        for bar, value in zip(bars, values):
            ax.text(bar.get_x() + bar.get_width() / 2, value + 0.008, pct(value), ha="center", va="bottom", fontsize=9)
        ax.set_title(f"Question {question}", weight="bold")
        ax.set_xticks(x, [CONDITION_LABELS[c] for c in CONDITIONS])
        ax.set_ylim(0, max(0.6, max(values) * 1.3))
        ax.grid(axis="y", alpha=0.25)
        ax.spines[["top", "right"]].set_visible(False)
    axes[0].set_ylabel("Mean NAE at h=20\n(lower is better)")
    fig.suptitle("r21 pilot final comparison at h=20 (GPT-5.6-luna)", fontsize=15, weight="bold")
...
def make_history_chart(summary: list[dict], out: Path) -> None:
    ...
    for ax, question in zip(axes, QUESTIONS):
        for condition in CONDITIONS:
            values = [select(summary, question, history, condition)["mean_nae"] for history in HISTORIES]
    ...
    fig.suptitle("r21 pilot exploratory learning curve: h=10 → h=20", fontsize=15, weight="bold")
...
def make_effect_heatmap(summary: list[dict], out: Path) -> None:
    ...
    for question in rows:
        arm = select(summary, question, 20, "arm")["mean_nae"]
        crm = select(summary, question, 20, "crm")["mean_nae"]
        nm = select(summary, question, 20, "nm")["mean_nae"]
        matrix.append([arm - crm, ((arm + crm) / 2) - nm])
    matrix = np.asarray(matrix)
    limit = max(0.06, float(np.abs(matrix).max()) * 1.15)
    fig, ax = plt.subplots(figsize=(7.5, 3.6))
    image = ax.imshow(matrix, cmap="RdBu_r", vmin=-limit, vmax=limit, aspect="auto")
    ax.set_xticks([0, 1], ["ARM − CRM", "Memory mean − NM"])
    ax.set_yticks(np.arange(len(rows)), [f"Q{q}" for q in rows])
    for i in range(matrix.shape[0]):
        for j in range(matrix.shape[1]):
            ax.text(j, i, f"{matrix[i, j]:+.3f}", ha="center", va="center", weight="bold")
    ax.set_title("r21 h=20 descriptive effect contrasts (GPT-5.6-luna)", fontsize=13, weight="bold")
...
def write_outputs(summary: list[dict], operational: dict, out: Path) -> None:
    ...
    def s(question: str, history: int, condition: str) -> float:
        return select(summary, question, history, condition)["mean_nae"]

    lines = [
        "# 项目1 · r21 技术试点可视化结果",
        "",
        f"- 题目：`4.13`、`5.7`；项目状态：`{operational['status']}`；完成时间：`{operational['completed_at']}`。",
        f"- 模型：`{operational['model']}`（{operational['cli_version']}，reasoning_effort={operational['reasoning_effort']}）。",
        f"- 试点执行：{operational['attempts']} 次 attempts，失败调用 {operational['failed_calls']}。",
        f"- 记忆约束检查：{operational['snapshot_count']} 个 snapshot，最大可见 token 数（近似）{operational['max_visible_tokens']}。",
        f"- 延迟：p50 {operational['latency_p50_ms'] / 1000:.2f}s，p95 {operational['latency_p95_ms'] / 1000:.2f}s。",
        "",
        "## 图表",
        "",
    ...
    for question in QUESTIONS:
        nm, crm, arm = (s(question, 20, c) for c in CONDITIONS)
        lines.append(f"| {question} | {nm:.4f} | {crm:.4f} | {arm:.4f} | {arm - crm:+.4f} | {((arm + crm) / 2) - nm:+.4f} |")
    lines += [
        "",
        "## 解释边界",
        "",
...
def main() -> None:
    ...
    output_dir = args.output_dir or repo_root / "outputs" / "r21_pilot_visualizations"
    ...
    print(f"Wrote r21 pilot visualizations to {output_dir}")
