"""REST endpoints for the expanded r22 technical pilot."""
...
from app.services.r21 import R21DomainError
from app.services.r22 import R22Service
...
def service(db: Session = Depends(get_db)) -> R22Service:
    return R22Service(db)
...
def _call(fn):
    try:
        ...
        status = {"not_found": 404, "conflict": 409, "validation": 422}.get(
            exc.code, 400
        )
        raise HTTPException(
...
def prompts(svc: R22Service = Depends(service)):
    return _call(svc.list_prompt_versions)


@router.post("/prompt-versions", status_code=201)
def create_prompt(payload: PromptPayload, svc: R22Service = Depends(service)):
...
def freeze_prompt(
    version_id: str, confirm: bool = Query(False), svc: R22Service = Depends(service)
):
    _confirm(confirm)
...
@router.get("/projects")
def projects(svc: R22Service = Depends(service)):
    return _call(svc.list_projects)


@router.post("/projects", status_code=201)
def create_project(payload: ProjectPayload, svc: R22Service = Depends(service)):
    return _call(lambda: svc.create_project(payload.model_dump()))


@router.get("/projects/{project_id}")
def project(project_id: str, svc: R22Service = Depends(service)):
    return _call(lambda: svc.get_project(project_id))


@router.post("/projects/{project_id}/freeze")
def freeze_project(
    project_id: str, confirm: bool = Query(False), svc: R22Service = Depends(service)
):
    _confirm(confirm)
...
def groups(project_id: str, svc: R22Service = Depends(service)):
    return _call(lambda: svc.list_groups(project_id))


@router.get("/projects/{project_id}/report")
def report(project_id: str, svc: R22Service = Depends(service)):
    return _call(lambda: svc.get_report(project_id))


@router.post("/projects/{project_id}/pause")
def pause(project_id: str, svc: R22Service = Depends(service)):
    return _call(lambda: svc.pause_project(project_id))


@router.post("/projects/{project_id}/resume")
def resume(project_id: str, svc: R22Service = Depends(service)):
