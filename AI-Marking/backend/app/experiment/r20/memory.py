"""Strict r20 v4 memory validation; invalid output is never repaired."""
...
_GRADE_POLICY = re.compile(
    ...
    r"\b\d+(?:\.\d+)?\s*(?:scores?|points?|marks?|credits?|%|percent(?:age)?)\b|"
...
def _contains_content(text: str, content_terms: set[str]) -> bool:
    return bool(set(_normal(text)) & content_terms)
...
def _contains_answer_copy(text: str, answer: str) -> bool:
    needles = token_ids(" ".join(_normal(text)))
    haystack = token_ids(" ".join(_normal(answer)))
...
def _validate_items(
    ...
    for item_index, item in enumerate(items, start=1):
        ...
        if condition == "crm":
            ...
            if detail_tokens > MAX_GUIDANCE_TOKENS:
                raise ValueError(
                    ...
                    "(about 14 tokens or fewer) and shorten the longest anchor."
                )
        ...
        for value in _string_values(item):
            if _GRADE_POLICY.search(value) or _contains_answer_copy(value, answer):
                raise ValueError("memory contains a grade policy or answer copy")

    visible = visible_token_count(condition, items)
    stored = stored_token_count(items)
...
def parse_memory_update(
    ...
    if condition == "crm":
        ...
        raise ValueError(f"unknown r20 memory condition: {condition}")
