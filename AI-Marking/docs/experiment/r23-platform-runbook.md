...
## 10. 统一实验平台（2026-09-05 起）

r23 已冻结为只读历史栈并完整迁入统一平台；新研究一律走统一核心。

- **接口**：`/api/experiments`（数据集、模板、runner、rubric、项目、生命周期、报告与导出）；
  r23 API 保留只读兼容入口，所有写操作返回 409 `read_only`（可用 `R23_READ_ONLY=false` 临时放开）。
- **前端**：默认入口 `/experiments`（数据集—模板—项目）；`/dress` 降级为 legacy 查看页，
  统一项目列表中的 legacy 项目直接链接到原 r23 详情页。
- **数据模型**：17 张 `exp_` 表（迁移 head `u7v8w9x0y1z2`）。正文只在 `exp_inputs` 存一份；
  评分通道由 `exp_scoring_contracts` 按数据集定义（CASE 网格 x2 2..10，New 网格 x2 1..10）；
  观测级模板（CASE）通过 `exp_observation_slots` + `exp_evaluation_slots` 表达
  "多个观测共享一次去重后调用" 的冻结结构。
- **注册的模板**：
  - `dress_new_human_agreement_v1`（2 个对齐模型；详见 `dress-new-human-agreement.md`）；
  - `dress_case_sensitivity_v1`（单模型，观测槽位；数据门/抽样/分析逐字委托冻结的
    `app.experiment.r23` 代码，golden 测试钉死一致性；跨维同文合并语义与 r23 相同）。
- **legacy 导入**：`.venv/bin/python -m scripts.import_legacy_r23`（幂等、r23 表零写入）。
  已导入 5 个项目：2 个试点（已封存）、luna/terra 正式（报告哈希逐字校验通过）、
  `sol正式`（paused、无报告，原样保留，不自动续跑）。
- **worker**：`./start.mcp.sh`（MCP 宿主）或 `.venv/bin/python -m scripts.run_worker`
  （独立串行 worker，适合多天运行）。两者共用同一全局租约，**任一时刻只运行一个**；
  轮转顺序：统一核心 → r23 → r21。
- **配额风险**：sol 正式项目曾因账号配额在 445/540 处中断（协议偏差 D1）。
  长时间运行请保持暂停/恢复与人工重试可用，并留意账号余额与失败告警。
- **数据集根**：统一核心读取 `EXP_DATASETS_ROOT`（默认 `SURF/DREsS`）；API 仍不接受任意路径。
